...
// SVG path "d"를 matter-js 정점 배열로 변환
...
const Gravity = forwardRef<GravityRef, GravityProps>(
  (
    ...
    const initializeRenderer = useCallback(() => {
      ...
      // Matter.Mouse는 wheel/touch 이벤트를 preventDefault(passive:false)로 잡아
      // 캔버스 위에서 페이지 스크롤을 통째로 막는다. 칩 드래그는 mouse 3종이면
      // 충분하므로 스크롤을 삼키는 리스너는 즉시 해제한다.
      ...
      mouseHandlers.element.removeEventListener("wheel", mouseHandlers.mousewheel)
      mouseHandlers.element.removeEventListener("touchmove", mouseHandlers.mousemove)
      mouseHandlers.element.removeEventListener("touchstart", mouseHandlers.mousedown)
      mouseHandlers.element.removeEventListener("touchend", mouseHandlers.mouseup)
