...
/**
 * 메인 대시보드 통계 (PRD §4.7).
 * - 주차별 점수 추이: 해당 주차 시험의 확정(final_score) 총점 — 멤버 전원 + 전체 평균
 * - 주차별 출석률 추이: (출석 + 지각×0.5) 기준 — 멤버 전원 + 전체 평균 (종료된 세션만)
...
function weeklyGroupScoreTrend(
  rows: ScoreInputRow[],
  memberIds: string[],
): GroupTrendPoint[] {
...
function weeklyGroupAttendanceTrend(
